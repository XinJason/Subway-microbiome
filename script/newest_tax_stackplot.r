...
if (TRUE){
  option_list = list(
    make_option(c("-t", "--type"), type="character", default="c",
    ...
    make_option(c("-n", "--group"), type="character", default="StationType",
...
colours <- c("#ebe7e4","#d1c7ae", "#c6e3d8", "#c8ccc1","#aabdc4","#e7cfd4", "#8d958f",  "#6e746a", "#48597e", "#8F7C00","#FFCC99", "#E69F00", "#cf9198", "#dB9c7c");
...
data_all$group <- factor(data_all$group,levels=c( "IHS", "UHS", "SHS", ordered=TRUE))
...
########################################################
#从做好的物种分类信息出发
tax_sample = read.table("result/tax/stackplot_c_group.txt", comment.char="", row.names= 1, header=T, sep="\t") 
...
data_all$variable  = factor(data_all$variable, levels=c("IHS", "UHS", "SHS", ordered=TRUE))
...
#这条命令是按自己颜色进行设置
scale_fill_manual(values=colours[1:22])
#theme_light(base_line_size = 0)
# 保存pdf和png格式方便查看和编辑
