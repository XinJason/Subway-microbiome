...
# 实验差异比较结果
x = read.table("result/compare/Warm-Cold.txt", header=T, row.names= 1, sep="\t", stringsAsFactors = F)
# 只提取前14列
x = x[,1:7]
x = na.omit(x)
...
p = ggplot(x, aes(x=num, y=neglogp, color=tax, size=log2CPM, shape=level)) +
  ...
  labs(title=paste(gsub(".txt", "", basename("result/compare/Warm-Cold.txt")), sep=" ")) +
  main_theme +
...
p
ggsave(file=paste("result/compare/Warm-Cold.txt.manhattan.pdf", sep=""), p, width = 180, height = 60, useDingbats=F, units = "mm")
# ggsave(file=paste("result/compare/Warm-Cold.txt_manhattan.png", sep=""), p, width = 180, height = 60)
