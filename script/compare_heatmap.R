...
# 读取比较列表
input = read.table("result/compare/Warm-Cold.txt", header=T, row.names=1, sep="\t", comment.char="", stringsAsFactors=F)
# 筛选显著差异部分
...
if (dim(norm)[1]>1){
  ...
  pheatmap(norm,
           ...
           filename = paste("result/compare/Warm-Cold.txt", ".heatmap.pdf", sep=""),
           width=10, height=14,
  ...
  #          annotation_col = anno_col,
  #          annotation_row = anno_row,
  #          filename = paste("result/compare/Warm-Cold.txt", ".heatmap.png", sep=""),
  #          width=10, height=14,
  #          annotation_names_row= T,annotation_names_col=T,
  #          show_rownames=T,show_colnames=T,
  #          fontsize=8,display_numbers=F)
  # # 提示工作完成
  # print(paste("Output in result/compare/Warm-Cold.txt", "_heatmap.pdf finished.", sep = ""))
}
