...
dir_path = paste("D:/DATA_PROCESS/Subway_Microbiome/Bacteria_Data/FEASTSourceTracker/")
setwd(paste0(dir_path, "FEAST_src"))
source("./src.R")

#Example data arguments 
metadata_file = "Pathogenmetadata.txt" #metadata file name
count_matrix = "Pathogen_otutab_rare.txt" #count_matrix file name
...
metadata<-metadata %>% 
  filter(Month == "Oct")
# Load OTU table
otus <- read.table(file = count_matrix, header=T, row.names=1, sep="\t", comment.char="" )
otus <- t(as.data.frame(otus))
...
write.table(Proportions_est, "Oct_FEAST_result.txt", quote = F, sep="\t", eol = "\n", na = "NA", dec = ".", row.names = T, col.names = T)
