...
index$group <- factor(index$group, levels=c("Intercity", "UrbanHub", "Residential", ordered=TRUE))
...
p0=ggplot(index0, aes(x=group, y=index0[[opts$type]], color=group),na.rm = TRUE) +
  geom_boxplot(alpha=1, outlier.size=0, size=0.7, width=0.5, fill="transparent") +
  labs(x="Beijingnan", y=paste(opts$type, "index"),fill="Temperature zone") + 
  geom_text(data=index, aes(x=group, y=y, color=group, label= stat)) +
  geom_jitter( position=position_jitter(0.17), size=1, alpha=0.5)+
  theme_bw(base_line_size = 1.05,base_rect_size = 1.05)+
...
color <- c("#226600","#3e5780","#594359")
##自己加的，调整分组顺序命令"#d1c7ae", "#8F7C00", "#104E8B", "#7E89A4","Winter", "Spring", "Summer", "Autumn"
alpha$Station <- factor(alpha$Station, levels=c("Intercity", "UrbanHub", "Residential", ordered=TRUE))
...
design1<-design[design$group2 == "Yearone",]
index1<-index[rownames(design1),]
index1<-as.data.frame(index1)
#df1<-df[df$group2 == "Year one" | env.data$Location == "middle",]
index1 <- na.omit(index1)
p1=ggplot(index1, aes(x=group, y=index1[[opts$type]], color=group)) +
  geom_boxplot(alpha=1, outlier.size=0, size=0.7, width=0.5, fill="transparent") +
  labs(x="Year one", y=paste(opts$type, "index"),fill="Temperature zone") + 
  geom_text(data=index, aes(x=group, y=y, color=group, label= stat)) +
  geom_jitter( position=position_jitter(0.17), size=1, alpha=0.5)+
  theme_bw(base_line_size = 1.05,base_rect_size = 1.05)+
  theme(axis.text=element_text(colour='black',size=9), legend.position = "none")+
...
##p2
design2<-design[design$group2 == "Yeartwo",]
index2<-index[rownames(design2),]
index2<-as.data.frame(index2)
#df1<-df[df$group2 == "Year one" | env.data$Location == "middle",]

p2=ggplot(index2, aes(x=group, y=index2[[opts$type]], color=group)) +
  geom_boxplot(alpha=1, outlier.size=0, size=0.7, width=0.5, fill="transparent") +
  labs(x="Year two", y=paste(opts$type, "index"),fill="Temperature zone") + 
  geom_text(data=index, aes(x=group, y=y, color=group, label= stat)) +
  geom_jitter( position=position_jitter(0.17), size=1, alpha=0.5)+
  theme_bw(base_line_size = 1.05,base_rect_size = 1.05)+
  theme(axis.text=element_text(colour='black',size=9), legend.position = "none")+
  ...
  theme(panel.grid.major=element_blank(),panel.grid.minor=element_blank())  
# 颜色可以自己设置，或者直接用scale_color_brewer()
p2=p2+ scale_color_npg()  

##p3
design3<-design[design$group2 == "Yearthree",]
index3<-index[rownames(design3),]
index3<-as.data.frame(index3)
#df1<-df[df$group2 == "Year one" | env.data$Location == "middle",]

p3=ggplot(index3, aes(x=group, y=index3[[opts$type]], color=group)) +
  geom_boxplot(alpha=1, outlier.size=0, size=0.7, width=0.5, fill="transparent") +
  labs(x="Year three", y=paste(opts$type, "index"),fill="Temperature zone") + 
  geom_text(data=index, aes(x=group, y=y, color=group, label= stat)) +
  geom_jitter( position=position_jitter(0.17), size=1, alpha=0.5)+
  theme_bw(base_line_size = 1.05,base_rect_size = 1.05)+
  theme(axis.text=element_text(colour='black',size=9), legend.position = "none")+
...
#图片组合
library(cowplot)
g=cowplot::plot_grid(p0,p1,p2,p3,ncol= 2, rel_widths = c(1, 1), labels=LETTERS[1:4])
ggsave(paste(opts$output, "Combined_TemZone_alpha.pdf", sep=""), g, width = opts$width, height = opts$height)
