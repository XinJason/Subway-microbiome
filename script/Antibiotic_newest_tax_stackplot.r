...
if (TRUE){
  option_list = list(
    make_option(c("-t", "--type"), type="character", default="mechenism",
    ...
    make_option(c("-b", "--number"), type="numeric", default=10,
  ...
  if (opts$input==""){opts$input=paste("result/sum_",opts$type,".txt", sep = "")}
  if (opts$output==""){opts$output=paste("result/stackplot_",opts$type,"",sep = "")}
  # 显示输入输出确认是否正确
...
colours <- c("#ebe7e4","#d1c7ae", "#e7f3ee","#c6e3d8", "#c8ccc1","#aabdc4","#e7cfd4", "#8d958f",  "#6e746a", "#48597e", "#8F7C00","#FFCC99", "#E69F00", "#cf9198", "#dB9c7c");
...
#data_all$group <- factor(data_all$group,levels=c( "Jan", "Feb", "Mar","Apr", "May", "Jun", "Jul", "Aug", "Sept", "Oct", "Nov", "Dec", ordered=TRUE))
...
p = ggplot(data_all, aes(x=variable, y = value, fill = tax )) + 
  ...
  #这条命令是按自己颜色进行设置
  scale_fill_manual(values=colours[1:10])+
...
########################################################
#从做好的物种分类信息出发
tax_sample = read.table("result/stackplot_mechenism_station_group.txt", comment.char="", row.names= 1, header=T, sep="\t") 
...
# 设置显示顺序，否则按字母排序(11会排在1后面)"Intercity", "UrbanHub", "Residential"
...
##绘制连线图
Palette <- c("#ebe7e4","#d1c7ae", "#e7f3ee","#c6e3d8", "#c8ccc1","#aabdc4","#e7cfd4", "#8d958f",  "#6e746a", "#48597e", "#8F7C00","#FFCC99", "#E69F00", "#cf9198", "#dB9c7c")
...
p3 <- p2 + scale_fill_manual(values = Palette[1:10])
#调整绘图区主题。
